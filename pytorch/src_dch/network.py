...
class ResNetFc(nn.Module):
  def __init__(self, name, hash_bit):
    ...
    self.weight_layer = nn.Linear(model_resnet.fc.in_features, self.component_num)
    self.weight_layer.weight.data.normal_(0, 0.01)
    self.weight_layer.bias.data.fill_(0.0)
...
class VGGFc(nn.Module):
  def __init__(self, name, hash_bit):
    ...
    self.hash_layer = nn.Linear(model_vgg.classifier[6].in_features, hash_bit*component_num)
    self.hash_layer.weight.data.normal_(0, 0.01)
    self.hash_layer.bias.data.fill_(0.0)

    self.weight_layer = nn.Linear(model_vgg.classifier[6].in_features, component_num)
    self.weight_layer.weight.data.normal_(0, 0.01)
    self.weight_layer.bias.data.fill_(0.0)
