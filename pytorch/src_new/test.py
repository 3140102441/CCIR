    ...
    if test_10crop:
        ...
        for i in range(len(loader[name+'0'])):
            ...
            for j in range(10):
                temp = model(inputs[j])
                l = []
                for cnt in range(inputs[j].size(0)):
                    l.append( torch.mm(temp[0][cnt].view(1,-1),torch.sign(temp[1][cnt].view(-1,hash_bit))))
                res = torch.cat(l,dim = 0)
                outputs.append(res)
...
def predict(config):
    ...
    test_codes, test_labels = code_predict(config["hash_bit"],dset_loaders, base_network, "test", test_10crop=prep_config["test_10crop"], gpu=use_gpu)
...
if __name__ == "__main__":
    ...
    config["snapshot_path"] = "../snapshot/"+config["dataset"]+"_"+str(args.hash_bit)+"bit_"+args.prefix+"/"+args.snapshot+"_ccir_model.pth.tar"
